...
def creatCVIndex():
    ...
    X_train,X_test,Y_train,Y_test=train_test_split(train,train.index,test_size=0.2,random_state=4)
    dumpCVIndex("../../data/cv/train.run4.txt", list(Y_train))
    dumpCVIndex("../../data/cv/valid.run4.txt", list(Y_test))
    X_train,X_test,Y_train,Y_test=train_test_split(train,train.index,test_size=0.2,random_state=5)
    dumpCVIndex("../../data/cv/train.run5.txt", list(Y_train))
    dumpCVIndex("../../data/cv/valid.run5.txt", list(Y_test))
    X_train,X_test,Y_train,Y_test=train_test_split(train,train.index,test_size=0.2,random_state=6)
    dumpCVIndex("../../data/cv/train.run6.txt", list(Y_train))
    dumpCVIndex("../../data/cv/valid.run6.txt", list(Y_test))
